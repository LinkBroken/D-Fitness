...
@login_required
def home():
    return render_template("homepage/home.html", user= current_user.username) # renders the "home.html" page  with "user" as an additional parameter

@views.route("/trainer") # routing the muscle group (trainer) page
@login_required
def trainer():
    return render_template("trainer/trainer.html", user=current_user.username)
    ...
    if logs: # if the user has any logs
        if request.method == "POST":
            try:
                ...
                if request.form["day"] == "":
                    flash("Fill in the day", category="error")
                elif request.form["month"] == "":
                    flash("Fill in the month", category="error")
                elif request.form["year"] == "":
                    flash("Fill in the year", category="error")
                flash("These Fields expect numbers", category="error")

    len_logs = 0
    return render_template("profile/profile.html", user=current_user, showing = logs, length = len_logs, day=workout_day)
    ...
    try:
        if request.method == "POST":
            ...
            try: # checking if the sets and reps are numbers
                ...
                flash(f"{exercise_name} added", category="error")
